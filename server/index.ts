  ...
  try {
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (error.code === 'EADDRINUSE') {
          console.error(`Port ${PORT} is already in use, trying port ${PORT + retries}`);
    ...
    setTimeout(async () => {
      try {
        ...
        // Try seeding contract templates
        try {
        ...
        console.error('Warning: Background data seeding failed:', error);
      }
    }, 1000);
