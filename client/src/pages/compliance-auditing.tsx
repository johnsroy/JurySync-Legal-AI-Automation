...
// Add QuickStats component to show immediate statistics
const QuickStats: FC<{ stats: ComplianceResult['quickStats'] }> = ({ stats }) => {
...
export const ComplianceAuditing: FC = () => {
  ...
  const { data: taskResult, isLoading: isLoadingTask, error: taskError } = useQuery<ComplianceResult>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        // Show quick stats even if full analysis is still processing
        if (data.status === 'processing' && data.data?.quickStats) {
          return data.data;
        }
...
const AnalysisResults: FC<{ result: ComplianceResult }> = ({ result }) => {
  ...
  // Helper function to map priority to badge variant
    ...
    <main className="container mx-auto px-4 py-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="space-y-4">
            {/* Quick Stats Display */}
            {taskResult?.quickStats && (
              <QuickStats stats={taskResult.quickStats} />
            )}
            ...
            {taskResult && taskResult.auditReport && (
              <AnalysisResults result={taskResult} />
            )}
